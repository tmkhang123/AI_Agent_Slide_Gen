...
def _strip_markdown(text: str) -> str:
    """Loại bỏ hoàn toàn các ký hiệu markdown khỏi chuỗi trước khi in."""
    if not text:
        return ""
...
class SlideGenerator:
    ...
    def _add_formatted_text(self, paragraph, text, pt, default_color, default_font="Calibri"):
        """Tách văn bản theo tag ***, ** hoặc * để bôi đậm, in nghiêng đúng vị trí và xóa sạch ký tự rác."""
        normalized = text.replace("<b>", "**").replace("</b>", "**")
        ...
        for part in parts:
            if part.startswith('***') and part.endswith('***'):
                val = part[3:-3]
                if val:
                    self._run(paragraph, val, pt, bold=True, italic=True, color=default_color, font=default_font)
            elif part.startswith('**') and part.endswith('**'):
                val = part[2:-2]
                if val:
                    self._run(paragraph, val, pt, bold=True, color=default_color, font=default_font)
            elif part.startswith('*') and part.endswith('*'):
                val = part[1:-1]
                if val:
                    self._run(paragraph, val, pt, italic=True, color=default_color, font=default_font)
            else:
                if part:
                    self._run(paragraph, part, pt, bold=False, color=default_color, font=default_font)
    ...
    def add_title_slide(self, title: str, subtitle: str,
        ...
        self._run(p, title_clean, 34, bold=True)

        # 6. Phụ đề
        tb2 = self._tb(slide, 0.55, 3.2, 5.7, 1.5)
        p2  = tb2.text_frame.paragraphs[0]
        p2.alignment = PP_ALIGN.LEFT
        self._run(p2, subtitle_clean, 15, italic=True, color=ICE_BLUE)
    ...
    def add_content_slide(self, title: str, bullet_points: list,
                          image_stream: BytesIO | None = None,
                          deck_title: str = "", page_num: int = 0):
        slide = self._blank()
        W, H  = self.W, self.H
        wp, hp = self._wp, self._hp

        title_clean = _strip_markdown(title)
        deck_title_clean = _strip_markdown(deck_title)

        # Bố cục thống nhất cho toàn deck (Visual Identity): Dark Navy Theme
        self._solid_bg(slide, NAVY)

        # ── Header bar ──
        tb_t = self._tb(slide, 0.5, 0.3, 9.0, 0.8)
        p_t  = tb_t.text_frame.paragraphs[0]
        p_t.alignment = PP_ALIGN.LEFT
        self._run(p_t, title_clean, 24, bold=True, color=WHITE)

        # Thanh phân cách ngang bên dưới tiêu đề
        self._rect(slide, 0.5, 1.15, 9.0, 0.03, ICE_BLUE)

        # ── Nội dung & Ảnh (Split Layout: chia đôi cột nếu có ảnh) ──
        BT = 1.35
        BH = 5.4
        ...
        for pt in bullet_points:
            ...
            # Ký tự đầu dòng ▪
            self._run(p, "▪  ", 15, bold=True, color=ICE_BLUE)
            # Nội dung chữ
            self._add_formatted_text(p, pt, 14, default_color=WHITE)

        # ── Footer hiển thị trang và tên bài thuyết trình (không lặp tiêu đề slide) ──
        tb_f = self._tb(slide, 0.5, H - 0.45, 9.0, 0.3)
        p_f  = tb_f.text_frame.paragraphs[0]
        p_f.alignment = PP_ALIGN.RIGHT
        
        display_title = deck_title_clean if deck_title_clean else "AI Slides Maker"
        footer_text = f"{display_title}   |   Trang {page_num}" if page_num > 0 else display_title
        self._run(p_f, footer_text, 8, color=MID_GRAY)
