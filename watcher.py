...
class JsonChangeHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if event.src_path.endswith("_content.json"):
            ...
            json_path = os.path.abspath(event.src_path)
            filename = os.path.basename(json_path)
            print(f"\n[DETECTED] Phát hiện thay đổi trong: {json_path}")
            print(f"[*] Đang tự động cập nhật Slide...")
            
            try:
                # Run main.py with the modified JSON path.
                subprocess.run(
                    [self.python_exe, self.script_path, json_path],
                    check=True,
